...
-- Sua query aqui
SELECT plataforma, COUNT(*) AS quantidade_jogos 
FROM games 
GROUP BY plataforma;
-- 8. Calcular a soma total de cópias vendidas de todos os jogos
-- Sua query aqui
SELECT SUM(copias_vendidas) AS total_copias_vendidas_global
FROM games;
-- 9. Calcular o preço médio dos jogos por género, ordenado do mais caro para o mais barato
-- Sua query aqui
SELECT genero, AVG(preco) AS preco_medio_genero
FROM games
GROUP BY genero
ORDER BY preco_medio_genero DESC;
